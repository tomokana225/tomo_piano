...
export const SettingsTab: React.FC<SettingsTabProps> = ({ uiConfig, onSaveUiConfig }) => {
    const { activeUserCount, dailyVisitorCount } = useApi();
    ...
    return (
        <div className="space-y-10">
            {/* 統計サマリー */}
            <section className="bg-gradient-to-br from-cyan-500/10 to-purple-500/10 p-6 rounded-2xl border border-cyan-500/20 shadow-inner">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                    <span className="w-2 h-6 bg-cyan-500 rounded-full"></span>
                    本日の統計
                </h3>
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/50 dark:bg-gray-800/50 p-4 rounded-xl border border-white/20">
                        <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1">現在の閲覧者</p>
                        <p className="text-3xl font-black text-cyan-600 dark:text-cyan-400">{activeUserCount}<span className="text-xs ml-1">人</span></p>
                    </div>
                    <div className="bg-white/50 dark:bg-gray-800/50 p-4 rounded-xl border border-white/20">
                        <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1">本日の総訪問者</p>
                        <p className="text-3xl font-black text-purple-600 dark:text-purple-400">{dailyVisitorCount}<span className="text-xs ml-1">人</span></p>
                    </div>
                </div>
            </section>
